...
class SimpleAttention(nn.Module):
    def __init__(self, model_hparams_dic):
        ...
        self.q_dense_layer = nn.Linear(self.embedding_dim,self.hidden_dim)
        self.k_dende_layer = nn.Linear(self.embedding_dim,self.hidden_dim)
        self.v_dense_layer = nn.Linear(self.embedding_dim,self.hidden_dim)
        self.Multihead    = nn.MultiheadAttention(embed_dim = self.hidden_dim, num_heads = self.num_MHA)
...
class TriggerCatAttention(nn.Module):
    def __init__(self, model_hparams_dic):
        ...
        self.trig_dim = self.hidden_dim

        self.trig_embedding_layer = nn.Embedding(2, self.trig_dim)
        self.q_dense_layer = nn.Linear(self.embedding_dim+self.trig_dim, self.hidden_dim)
        self.k_dende_layer = nn.Linear(self.embedding_dim,   self.hidden_dim)
        self.v_dense_layer = nn.Linear(self.embedding_dim,   self.hidden_dim)
        self.Multihead    = nn.MultiheadAttention(embed_dim = self.hidden_dim, num_heads = self.num_MHA)
...
class RelationExtractionModule(nn.Module):
    def __init__(self, vocab, model_hparams_dic, rel_dic):
        ...
        self.rel_conv         = nn.Conv1d(self.hidden_dim, self.hidden_dim, self.window_size, padding=((self.window_size//2), ))
        self.rel_conv2        = nn.Conv1d(self.hidden_dim, self.hidden_dim, self.window_size, padding=((self.window_size//2), ))

        # self.rel_linear      = nn.Linear(self.hidden_dim, self.rel_size)
        self.linearhead          = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.lineartail          = nn.Linear(self.hidden_dim, self.hidden_dim)
        ...
        for b, pair_list in data_unit_for_relation.items():
            if len(pair_list) > 0:
                for one_pair in pair_list:
                    ...
                    headrep = F.relu(self.linearhead(headrep))
                    tailrep = F.relu(self.lineartail(tailrep))
...
class MyModel(nn.Module):
    ...
    def forward(self, epoch, n_doc, tokens_tensor, trigger_vecs, padding_mask, NER_RE_switch, y_spans, Relation_gold_learning_switch, i, is_training, is_share_stop):
        ###  SHARE PART ###
        with torch.no_grad():
        ...
        if NER_RE_switch == "NER": #NERの学習のみを行う場合
            ...
            if epoch >3:
                if is_training:
                    self.logit_dic_tr[i] = torch.max(logits_spans, dim=2)[1].permute(1,0,2).permute(1,0,2).cpu().numpy()
                else:
                    self.logit_dic_dev[i] = torch.max(logits_spans, dim=2)[1].permute(1,0,2).permute(1,0,2).cpu().numpy()
            return logits_spans
        ...
        if NER_RE_switch == 'RE':
            if is_share_stop:
                ...
                if is_training:
                    logits_spans = self.logit_dic_tr[i] 
                else:
                    logits_spans = self.logit_dic_dev[i]
